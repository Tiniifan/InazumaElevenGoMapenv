...
def compile_mapenv(file_path):
    ...
    file_data = bytes(16)  # Placeholder for file data
    ...
    for line in lines:
        ...
        if line.startswith("PTREE"):
            ...
            values = line[6:][:-1].replace('"', '').split(',')  # Extract PTVAL values
    ...
    file_data = align_bytes(file_data, 0xFF, 16) + text_header + b"\x00"  # Align bytes, add text header and null terminator
    file_data = align_bytes(file_data, 0xFF, 16)  # Align bytes again
    ...
    for i in range(entries_count):
        ...
        if current_tag == b'\xa2T\xde^':
            ...
            if tag_size == 2:
                ...
                text_output += ' ' * (indent_level * 4) + 'PTREE "MAPENV","' + text + '";\n'
